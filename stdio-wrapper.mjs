...
const isCancelledNotification = (value) =>
...
export function proxyTransports(clientTransport, serverTransport) {
    let closed = false;
    const propagateClose = (source, target) => {
        source.onclose = () => {
            if (!closed) {
                closed = true;
                target.close();
    ...
    const propagateMessage = (source, target) => {
...
let url = `http://localhost:${PORT}${mcpPath}`;
...
app.post(mcpPath, authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (transport) {
      ...
      // Handle TypeScript files by prepending tsx
      let command = args[0];
      ...
      const clientTransport = new StdioClientTransport({
        command: command,
        args: commandArgs,
        env: process.env,  // Pass all environment variables to the subprocess
      });
...
// File can now use top-level await with .mjs extension
