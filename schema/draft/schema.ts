...
/**
 * Used by the client to reconnect to or extend the keep alive of a call tool request running on the server.
 * 
...
export interface JoinCallToolAsyncRequest {
  ...
  params: {
    token: AsyncToken
    /**
     * The number of seconds to keep results and resources generated by this call tool available on the server
     */
    keepAlive?: number
  };
}
...
/**
 * Used by the client to cancel a previous async tool call request.
 * 
 * The request SHOULD still be in-flight, but due to communication latency, it is always possible that this request MAY arrive after the request has already finished.
 *
...
/**
 * Used by the server to provide a reference to the client for a previously submitted async tool call.
 * 
...
export interface ServerCapabilities {
  ...
  /**
   * Present if the server offers async tool calls.
   */
  async?: {
    /**
     * The maximum keep alive time in seconds that the server will support for async tool calls.
     */
    maxKeepAliveTime?: number
  }
}
