...
| [LLM as a Service][LLMAsAService]           | ❌          | ❌        | ✅      | ❌         | ❌    | Supports tools for browser-based Agents using SSE, Streaming HTTP transport |
...
[LLMAsAService]: https://llmasaservice.io
...
### LLM as a Service
LLM as a Service allows customers to create, deploy, and manage LLM features and chat agents in applications and websites through a unified control panel. MCP servers (using SSE/Streaming HTTP transport) can be attached to agents, extending their capabilities on the fly—no code changes or redeployments required.

**Key features:**
- Seamlessly add MCP server capabilities to chat agents
- Real-time SSE/Streaming HTTP transport for dynamic tool invocation
- Instantly add/change MCP agent functionality without redeployment 

Learn more:
[Demo video](https://youtu.be/ucEHsxuPNFo) and [Website](https://llmasaservice.io)
