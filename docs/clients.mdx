...
| [MCP Router][MCP Router]                         | ✅          | ✅        | ✅      | ❓                     | ❌         | ❌    | Supports tools, prompts, and resources.                                                         |
...
[MCP Router](https://github.com/mcp-router/mcp-router) is a free MCP management application available for MacOS and Windows.

**Key features:**

- **Ease of Use**: Quickly import MCP servers from JSON configurations, easily enable or disable servers, and conveniently view request logs.
- **Privacy-focused**: All data remains stored locally.
- **Transparent**: Source code available on GitHub for full transparency.
- **Flexible Integration**: Offers local data API without lock-in, ensuring full control of your data.
- **Universal Compatibility**: Connect seamlessly with any MCP server type (including Stdio, SSE, and Streamable).
- **Feature-rich**: Provides comprehensive request logs, local API access, and customizable parameters for all tools.
