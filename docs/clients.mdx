...
| [Saiki][Saiki]                              | ✅          | ✅        | ✅      | ❓                     | ❌          | ❌    | Supports tools, resources, prompts and multiple servers.       |
...
[Saiki]: https://github.com/truffle-ai/saiki
...
[Saiki](https://github.com/truffle-ai/saiki) is a flexible, customizable AI agent that fully supports the Model Context Protocol. Talk to Saiki in natural language to orchestrate any MCP‑compatible servers.

**Key features:**

- **Open & Extensible**: Drop in pre‑built or custom MCP servers to connect to any service via `stdio`, `sse` or `http`.  
- **LLM Agnostic**: Use any LLM model (Anthropic, OpenAI, Gemini, local, etc.)
- **Orchestration**: Figures out which tools to call—and in what order—based on your instruction.  
- **Multi‑Interface Support**: Use via CLI, APIs, embed in a web UI, or integrate into other applications such as Slack, Telegram, etc.  
- **Local‑First**: Run entirely on your machine or self‑host for full control.
