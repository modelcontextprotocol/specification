...
## TL;DR: these 3 lines to make your local MCP server accessible on the web to anyone with a secret URL (final code of this tutorial: [stdio-wrapper.ts](https://github.com/modelcontextprotocol/typescript-sdk/blob/ochafik/stdio-wrapper-example/src/examples/proxy/stdio-wrapper.ts)):
...
Before we start, we'll install our two main dependencies:

```bash
npm install -g @modelcontextprotocol/sdk express
```

(For the sake of simplicity we've used a global install but typically you'll want a local install in a folder in which you've run `npm init` and written a node.js-friendly `tsconfig.json`)

## Plugging transports together

Our proxy will spawn a subprocess that communicates w/ our wrapped MCP server over an stdio transport and will spawn an Express HTTP server that exposes a Streamable HTTP transport endpoint.
...
Let's stub a new file `stdio-wrapper.ts` with a few imports / utilities we'll need in this tutorial:
