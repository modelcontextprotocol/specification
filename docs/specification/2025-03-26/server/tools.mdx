...
- `outputSchema`: Optional JSON Schema defining the structure of JSON responses
...
The JSON content type allows structured data to be returned from tools. The `schema` field can be:
- Omitted (in which case the tool's `outputSchema` is implied)
- A string (which is interpreted as a URI reference to a schema)
- An object (containing the full schema definition)
