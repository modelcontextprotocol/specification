...
1. MCP authorization servers and MCP servers **SHOULD** support the OAuth 2.0 Dynamic Client Registration
...
MCP clients **MUST** handle the `WWW-Authenticate` header.
...
version. MCP servers **SHOULD** use the `MCP-Protocol-Version` header to determine compatibility with the MCP client.
...
response. MCP client **MUST NOT** send tokens other than issues by the MCP server.
MCP servers **MUST** issue tokens only for valid for the MCP server. They **MUST NOT** pass
on any third-party token.
