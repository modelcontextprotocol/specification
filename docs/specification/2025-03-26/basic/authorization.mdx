...
- OAuth 2.0 Protected Resource Metadata IEFT DRAFT ([draft-ietf-oauth-resource-metadata-13](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-resource-metadata-13))
...
1. MCP auth implementations **SHOULD** support the OAuth 2.0 Dynamic Client Registration
...
1. MCP servers **SHOULD** and MCP clients **MUST** implement OAuth 2.0 Authorization
...
The authorization server is beyond the scope of this specification. It may be the same server as the
...
MCP servers **MAY** use the HTTP header `WWW-Authenticate` when returning a _401 Unauthorized_ to indicate the location of the authorization
server as described in [OAuth 2.0 Protected Resource Metadata](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-resource-metadata-13).

MCP clients **MUST** handle the `WWW-Authenticate` header if present, and otherwise use direct discovery via the well-known URI if `WWW-Authenticate`.
...
sequenceDiagram
    ...
    Note over C: Extract authorization_server from WWW-Authenticate
