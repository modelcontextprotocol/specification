...
When browser-based clients need to connect to MCP servers:

1. Servers **SHOULD** include the `Mcp-Session-Id` header in the `Access-Control-Expose-Headers` response header to allow browser-based clients to read the session ID
2. Servers **SHOULD** configure appropriate CORS policies including:
   - Setting `Access-Control-Allow-Origin` to appropriate origins (avoid using `*` in production)
   - Including `GET`, `POST`, and `DELETE` in `Access-Control-Allow-Methods`
   - Exposing the `Mcp-Session-Id` header via `Access-Control-Expose-Headers`

Without proper CORS configuration, browser-based clients will not be able to:
- Access the `Mcp-Session-Id` header from initialization responses
- Establish sessions with the MCP server
