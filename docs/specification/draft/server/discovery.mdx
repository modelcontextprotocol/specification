...
sequenceDiagram
    ...
    User->>Client: Provides URL https://notion.so/doc/123
    Client->>Domain: GET https://notion.so/.well-known/mcp.json
    Domain-->>Client: Returns server metadata
    Client->>User: "Notion has an MCP server available. Connect?"
