...
6. If the server initiates an SSE stream:
   ...
   - The server **SHOULD NOT** close the SSE stream before sending the JSON-RPC _response_
     for the received JSON-RPC _request_, unless it has sent a
     [resume policy notification](/specification/draft/basic/lifecycle#resuming-requests)
     or the [session](#session-management) expires.
