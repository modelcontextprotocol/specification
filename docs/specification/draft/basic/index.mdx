...
- Params **MAY** include a `_meta` key with value type `[key: string]: unknown;`. This
  allows propagation of metadata from client to server, such as [`progressToken`](./utilities/progress.mdx).
  OpenTelemetry SDKs [default](https://opentelemetry.io/docs/languages/sdk-configuration/general/#otel_propagators) to propagate W3C `traceparent`,`tracestate` and `baggage`
