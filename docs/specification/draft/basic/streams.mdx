...
1. **Creation**: The server creates a stream and sends a `stream/create` notification with a unique stream ID
2. **Live**: Messages are exchanged on the stream.
3. **Completion**: Server sends a `stream/end` notification when the stream is complete
