...
#### What if requests are sent without the initialization being acknowledged in the server?

If the client sends a request before initialization is completed (which may also be the case when a 
server looses the session in a restart when not using session persistence or a session timeout), the 
server **MUST** respond with a json error code of `-32080`.

Clients **MUST** properly initialize in this case. If the client had a previous session
running, it **MUST** consider that any previous session is invalid and that the server may have 
changed  its capabilities and even protocol version and do a new initialization.
...
- Initialization not completed (json error code `-32080`)
