...
MCP servers may need to access authenticated upstream resources on behalf of users. For these scenarios, MCP provides the [delegated authorization primitive](/specification/draft/client/delegated_authorization) which allows servers to request OAuth authorization from clients.

This complements the authorization flow described in this document:

- **Client Authorization** (this document): How MCP clients obtain authorization to access MCP servers
- **Delegated Authorization** ([/client/delegated_authorization](/specification/draft/client/delegated_authorization)): How MCP servers request authorization FROM MCP clients for upstream resources

Both specifications follow OAuth 2.1 best practices and work together to enable secure authentication flows in both directions.
