...
- **Consistent client identification**: Software statements provide a consistent and trustworthy way to identify MCP clients, making it easier for authorization servers to manage them.
- **Accountability**: Software statements establish a chain of trust back to the software vendor, supporting effective auditing.
...
To prevent replay attacks, MCP clients **MUST** include the authorization server's issuer URL in the software statement JWT claims. 
The issuer URL **MUST** match the `issuer` field from the authorization server's metadata. 
Authorization servers **MUST** verify that the issuer claim in the software statement matches their own issuer URL and **MUST** reject any software statements that do not include a matching issuer claim.
...
sequenceDiagram
    ...
    C->>CB: Request software_statement JWT with AS issuer
    Note over CB: Generate JWT with claims:<br/>- client metadata<br/>- software ID<br/>- software version<br/>- expiration<br/>- AS issuer URL
    ...
    CB-->>A: JWKS document
    Note over A: 1. Validate software_statement<br/>2. Verify JWT signature using JWKS<br/>3. Check issuer trust<br/>4. Validate claims & expiry<br/>5. Verify AS issuer claim matches
    A-->>C: Client credentials
```
