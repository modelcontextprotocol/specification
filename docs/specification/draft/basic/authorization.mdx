...
MCP servers may need to access authenticated upstream resources on behalf of users. For these scenarios, MCP provides the [authentication primitive](/specification/draft/client/authentication) which allows servers to request OAuth authentication from clients.

This complements the authorization flow described in this document:

- **Authorization** (this document): How MCP clients authenticate TO MCP servers
- **Authentication** ([/client/authentication](/specification/draft/client/authentication)): How MCP servers request authentication FROM MCP clients for upstream resources
