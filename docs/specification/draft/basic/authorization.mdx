...
When authorization servers support OpenID Connect (OIDC) and Dynamic Client Registration, they may enforce additional constraints on redirect URIs
based on the `application_type` parameter as defined in
[OpenID Connect Dynamic Client Registration 1.0](https://openid.net/specs/openid-connect-registration-1_0.html).

MCP clients **SHOULD** specify an appropriate `application_type` during Dynamic Client Registration:

- **Native applications** (desktop applications, mobile apps, CLI tools) **SHOULD** use `application_type: "native"`
- **Web applications** (browser-based applications) **SHOULD** use `application_type: "web"`

##### Authorization Server Behavior

Authorization servers implementing OIDC **MAY** enforce these constraints and **MAY** reject client registration requests
that violate the redirect URI requirements for the specified application type, as defined in the
[OpenID Connect Dynamic Client Registration 1.0 "Client Metadata" section](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).

MCP clients **SHOULD** be prepared to handle registration failures due to redirect URI constraints and may need to adjust their `application_type` or redirect URIs accordingly.
