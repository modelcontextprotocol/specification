...
This specification defines baseline requirements for interoperability between MCP clients and
servers. Implementations **MAY** offer additional authorization mechanisms beyond those
specified here. However, implementers should note that extending beyond the specification may
require custom client implementations rather than standard MCP clients.
...
1. Authorization Code Flow - Used when an end-user needs to authenticate via a user agent
   ...
   - Enables user-interactive authentication and consent
   - Supports both confidential and public clients

2. Client Credentials Flow - Used when a client needs to authenticate itself without user
   interaction. This flow:
   - Enables machine-to-machine authentication
   - Requires client authentication using either:
     - Asymmetric keys (JWT authentication) as defined in [RFC 7523 Section 2.1](https://datatracker.ietf.org/doc/html/rfc7523#section-2.1)
...
The complete Authorization Code flow proceeds as follows:
...
The Client Credentials flow enables machine-to-machine authentication without user interaction.
This flow requires pre-registered client credentials, which are typically established out-of-band
through administrative channels. Dynamic Client Registration is not used in this flow.
...
- JWT Authentication (RECOMMENDED)
  - Clients use JWT Authentication as defined in [RFC 7523 Section 2.1](https://datatracker.ietf.org/doc/html/rfc7523#section-2.1).
- Client Secret
  - Clients use a Client Secret with HTTP Basic authentication as defined in [OAuth 2.1 Section 2.4.1](https://www.ietf.org/archive/id/draft-ietf-oauth-v2-1-13.html#name-client-secret)
