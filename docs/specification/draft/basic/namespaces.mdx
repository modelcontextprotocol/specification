...
## Capability
Although it is not required, since namespaces at a high level are backwards compatible with names, to indicate support
for feature level advanced filtering, the server **MAY** expose a namespace capability:
```json
{
  "capabilities": {
    "namespaces": {}
...
## Namespace Format
A namespace must be a string of the form `@<namespace>` where `<namespace>` is any of alphanumeric characters
(A-Z, a-z, 0-9), underscores, dots (.), and hyphens. For instance, @com.github would be an acceptable namespace.
