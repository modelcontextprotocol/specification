...
MCP implements a client-server architecture where an MCP host (usually an AI application such as [Claude Code](https://www.anthropic.com/claude-code) or Cursor) establishes connections to one or more MCP servers. An MCP host instantiates an MCP client per MCP server. Each MCP client maintains a one-to-one connection with its corresponding MCP server.
