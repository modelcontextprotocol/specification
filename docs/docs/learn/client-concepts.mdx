...
MCP clients are protocol clients created by host applications to communicate with MCP servers. While the host application (like Claude.ai or Code Editor) manages the overall user experience and coordinates multiple clients, each client handles the direct communication with servers through the Model Context Protocol.

Understanding the distinction is important: the host is the application users interact with, while clients are the protocol-level components that enable server connections. For stateful servers, each client maintains a dedicated connection session. For stateless servers accessed over HTTP, clients may share connections while still maintaining logical separation of concerns and security boundaries.

## Core Client Features

Clients provide several key features that enhance server capabilities:
...
Sampling is a standardized mechanism that enables servers to implement complex behaviors by requesting AI model interactions through the client. Rather than servers needing their own API keys or model integrations, they can request completions in a controlled manner. The client handles model selection based on the server's preferences (cost, speed, or intelligence priorities), manages user approval flows, and ensures security boundaries. This nested capability means sampling requests occur within the context of other operations—like a tool analyzing data or a resource being processed—always with human oversight.
...
The tool queries airline APIs and gathers dozens of flight options. It then requests AI assistance to analyze these options: "Analyze 47 flights to Barcelona considering price, duration, layovers, and departure times. User prefers morning flights with one layover maximum."

The client asks the user: "Allow AI to analyze flight options?" Upon approval, the AI evaluates trade-offs—like cheaper red-eye flights versus convenient morning departures. The tool uses this analysis to present the top three recommendations.
...
When the agent creates a Barcelona itinerary, the server works within these boundaries—accessing templates, saving the new itinerary, and referencing client documents. It cannot access files outside these roots.
...
Elicitation provides a structured way for servers to gather necessary information on-demand. Instead of requiring all information upfront or failing when data is missing, servers can pause their operations to request specific inputs from users. This creates more flexible interactions where servers adapt to user needs rather than following rigid patterns.
...
A travel booking server demonstrates elicitation's power through the final booking confirmation process. When a user has selected their ideal vacation package to Barcelona, the server needs to gather final approval and payment details before proceeding.

The server elicits booking confirmation with a summary: Barcelona flights June 15-22, beachfront hotel, total $3,000. Based on the response, it might request additional preferences—seat selection, room type, or travel insurance.
