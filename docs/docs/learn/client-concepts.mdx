...
In addition to making use of server features via *client-initiated requests*, clients may optionally provide several key features that enhance server capabilities, via *server-initiated requests*:
...
**Security and isolation**: Sampling requests are isolated from the main conversation context by default. Servers cannot access user conversations or sensitive data unless explicitly included through `includeContext`. Each sampling request is sandboxed, preventing servers from exfiltrating data or compromising the client's security boundaries.
