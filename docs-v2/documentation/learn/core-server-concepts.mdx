...
When working with dynamic resources, MCP helps you fill in the parameters. For example, if a travel server has a template like `hotel://search/{city}/{date}`, as you type the city name "Par...", it might suggest "Paris" or "Park City" from available destinations. This intelligent assistance helps you quickly access the exact resource you need without memorizing specific formats.
...
- **"Plan a vacation"** - Guides you through destination, dates, budget, and preferences
- **"Show weather for [city]"** - Displays current conditions and forecast for any destination
- **"Create daily schedule"** - Organizes activities, restaurants, and transportation for each day
...
Prompts get even better with auto-completion. When you select a prompt like "Show weather for [city]", MCP helps you complete the arguments. As you type "Par...", it might suggest "Paris, France" or "Park City, Utah". Similarly, with "Send email to [recipient]", typing "Joh..." could auto-complete to "John Smith" from your contacts. This works just like search suggestions in your favorite apps, making it easy to provide the right information quickly.
...
**In our vacation example**, tools enable actions like:

- **`searchFlights(origin, destination, dates)`** - Compare options across airlines
- **`checkHotels(city, checkin, checkout, budget)`** - Find available accommodations
- **`createCalendarEvent(title, date, location)`** - Block your travel dates
- **`sendEmail(to, subject, body)`** - Coordinate with travel companions
- **`createDocument(title, content)`** - Generate itineraries or packing lists
...
3. **You fill in the prompt parameters** with smart assistance:

   - Destination: "Barcelona" (auto-completed from "Bar...")
   - Dates: "June 15-22" (suggested based on your calendar availability)
   - Budget: "$3000"
   - Preferences: "Beach, local cuisine, walkable neighborhoods"
