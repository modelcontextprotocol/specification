...
Tools transform AI from an advisor into an assistant that can help execute tasks. Consider a travel planning scenario where the AI application helps book a vacation:

1. **`searchFlights(origin: "NYC", destination: "Barcelona", date: "2024-06-15")`**
   - Searches across multiple airlines
   - Returns structured flight options

2. **`createCalendarEvent(title: "Barcelona Trip", startDate: "2024-06-15", endDate: "2024-06-22")`**
   - Blocks calendar time
   - Adds trip details

3. **`sendEmail(to: "team@work.com", subject: "Out of Office", body: "...")`**
